...
// Memoria constante (solo lectura en device)
__constant__ float d_cos[DEGREE_BINS];
__constant__ float d_sin[DEGREE_BINS];

// KERNEL: versión con memoria CONSTANTE
__global__ void houghTransformKernelConst(
...
int main(int argc, char** argv)
{
    ...
    houghTransformKernelConst<<<blocks, threads>>>(
    ...
    printf("Tiempo de ejecución (memoria constante): %.3f ms\n", ms);
    ...
    writePPM("output_const.ppm", rgb, width, height);
    printf("-> Imagen generada: output_const.ppm\n");

    // Reporte de tiempos
    mkdir("reporte", 0777);
    FILE* rep = fopen("reporte/tiempos_const.txt", "a");
    if (rep) {
        fprintf(rep, "----------------------------------------\n");
        fprintf(rep, "Reporte de ejecución: Hough (memoria CONSTANTE)\n");
        ...
        fprintf(rep, "----------------------------------------\n\n");
        fclose(rep);
        printf("-> Reporte guardado en reporte/tiempos_const.txt\n");
    } else {
        printf("-> No se pudo crear reporte/tiempos_const.txt\n");
    }
